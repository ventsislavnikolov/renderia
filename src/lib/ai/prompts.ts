...
/**
 * Base objective prompt for the Room Composite synthesis. The input images are
 * the approved per-angle Structural Previews; the pipeline stitches the
 * captured arc into one continuous wide panorama of the empty room. It must
 * NOT invent walls that were never photographed and must NOT add furniture —
 * this is still an empty-room confirmation image, just covering the whole
 * captured room instead of one angle.
 *
 * This string is the shared "objective" half of every step in the progressive
 * outpaint loop (anchor + extensions) and is also persisted on the
 * `room_composites` row for provenance. The per-step scaffolding lives in
 * `buildRoomCompositeAnchorPrompt` / `buildRoomCompositeExtendPrompt`. See
 * docs/adr/0002.
 */
export function buildRoomCompositePrompt(input: {
	...
	return [
		...
		`Combine the supplied empty-room angle previews into ONE continuous wide panorama of the empty room for ${sanitizePromptField(input.taskTitle)}.`,
		`- Every input image is an empty-room preview of the same room from a different angle (${input.sourcePreviewCount} angle(s)).`,
		"- The result is a single wide panoramic image that reads as the whole room as captured, with the angles stitched into one coherent space.",
		...
		"",
		"OUTPUT RULES:",
		"- Photorealistic empty room, consistent lighting and perspective across the stitched view.",
...
/**
 * Per-step prompt for the FIRST tile of the panorama (the leftmost angle).
 * `gpt-image-2` caps output at 1536×1024, so the wide composite is built by
 * stitching tiles: this renders the anchor angle as one clean 3:2 empty-room
 * frame that every later tile extends rightward from.
 */
export function buildRoomCompositeAnchorPrompt(baseObjective: string) {
	return [
		baseObjective,
		"",
		"THIS STEP — ANCHOR TILE:",
		"- Render the supplied empty-room angle as one clean, photorealistic 3:2 empty-room frame.",
		"- Keep the walls, floor, ceiling, openings, proportions, and lighting exactly as shown; do not invent anything off-frame.",
		"- This frame is the left end of a wider panorama, so keep the right edge a natural place for the room to continue.",
	].join("\n");
}

/**
 * Per-step prompt for each EXTENSION tile. The supplied canvas has an
 * already-rendered strip of the panorama on the LEFT (which must stay
 * untouched — it is masked as keep) and the next photographed angle pasted on
 * the RIGHT (the masked-for-regeneration region). The model must redraw the
 * right region as a seamless continuation of the room that realises the angle
 * shown there.
 */
export function buildRoomCompositeExtendPrompt(baseObjective: string) {
	return [
		baseObjective,
		"",
		"THIS STEP — EXTEND THE PANORAMA RIGHTWARD:",
		"- The LEFT strip of this image is already-rendered room. Treat it as fixed truth and keep it perfectly continuous with what you draw.",
		"- The RIGHT region shows another photographed angle of the SAME empty room. Redraw it as a clean, photorealistic continuation of the room extending to the right.",
		"- Match wall colour, floor, ceiling line, perspective, and lighting to the left strip so the seam is invisible.",
		"- Preserve any window, door, or opening visible in the right region, in the same position and proportion.",
		"- Keep it an EMPTY room: no furniture, decor, or staging. Do not invent walls or openings the angle does not show.",
	].join("\n");
}

/**
 * Prompt for the vision pass that orders the angle previews left→right around
 * the room so the progressive outpaint stitches them in a spatially coherent
 * sweep rather than in arbitrary upload order.
 */
export function buildOrderRoomAnglesPrompt(angleCount: number) {
	return [
		"These images are photos of the SAME empty room, each taken from a different angle.",
		"Determine the left-to-right order in which the walls connect as you pan across the room, so the images can be stitched into one continuous panorama.",
		"Use spatial cues — shared wall edges, the positions of windows/doors/openings, and the floor and ceiling lines — to infer which angle sits next to which.",
		`Return an object { order: [...] }: a permutation of the indices 0..${angleCount - 1}, listing the images from the leftmost wall to the rightmost.`,
	].join("\n");
}
